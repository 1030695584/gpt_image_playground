...
import { enforcePresetConfigPolicy, isPresetConfigOnlyEnabled, isPresetConfigParamsLocked } from './presetConfig'
 ...
 */
function buildPresetConfigOnlySettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
...
function buildRegularSettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  ...
  const hasLegacyOpenAIParams = profileIdParam !== null || apiUrlParam !== null || apiKeyParam !== null || codexCliParam !== null || apiMode !== undefined || modelParam !== null || profileNameParam !== null || reasoningEffortParam !== null || streamImagesParam !== null || streamPartialImagesParam !== null
  ...
  if (hasLegacyOpenAIParams) {
    ...
    const existingById = requestedProfileId ? settings.profiles.find((item) => item.id === requestedProfileId) : null
    if (existingById) {
      return normalizeSettings({
        ...settings,
...
export function buildSettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  if (isPresetConfigOnlyEnabled() && isPresetConfigParamsLocked()) return {}

  const result = isPresetConfigOnlyEnabled()
    ? buildPresetConfigOnlySettingsFromUrlParams(currentSettings, searchParams)
    : buildRegularSettingsFromUrlParams(currentSettings, searchParams)
  if (!isPresetConfigParamsLocked() || Object.keys(result).length === 0) return result

  return enforcePresetConfigPolicy(normalizeSettings({
    ...normalizeSettings(currentSettings),
    ...result,
  }))
}
